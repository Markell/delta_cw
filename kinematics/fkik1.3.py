...
class DeltaRobotGUI:
    def __init__(self):
        ...
        self.us_per_degree = 7.333333
        self.us_offset = 1500          # можно подстроить позже
    ...
    def angle_to_microseconds(self, angle_deg):
        """Преобразует угол в градусах в микросекунды с ограничением"""
        us = self.us_offset + angle_deg * self.us_per_degree
        # Ограничение диапазона
        us = max(1500, min(2250, us))
        return int(round(us))
    ...
    def send_command(self, theta):
        if not self.ser or not self.ser.is_open:
            ...
            return False
        try:
            us1 = self.angle_to_microseconds(theta[0])
            us2 = self.angle_to_microseconds(theta[1])
            us3 = self.angle_to_microseconds(theta[2])

            cmd = f"US1:{us1} US2:{us2} US3:{us3}\n"
            self.ser.write(cmd.encode())
            print(f"Отправлено: {us1}, {us2}, {us3} мкс")  # для отладки
            return True
        ...
        try:
            ...
            if self.send_command(theta):
                messagebox.showinfo("Отправлено", 
                    f"Углы: {theta[0]:.2f}°, {theta[1]:.2f}°, {theta[2]:.2f}°\n"
                    f"Микросекунды: {us_values[0]}, {us_values[1]}, {us_values[2]}")
        except Exception as e:
    ...
    def send_microseconds(self):
        if not self.ser or not self.ser.is_open:
            ...
            return
        try:
            us = [max(1500, min(2250, var.get())) for var in self.servo_vars]
