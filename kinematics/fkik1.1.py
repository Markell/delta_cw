...
class DeltaRobotGUI:
    ...
    def create_widgets(self):
        # Заголовок
        ttk.Label(self.root, text="Delta Robot Control", font=("Arial", 18, "bold")).pack(pady=8)
        ...
        ttk.Button(ik_frame, text="Рассчитать IK + Отправить на сервоприводы", 
                  command=self.calculate_ik_and_send).grid(row=0, column=6, padx=15, pady=5)

        self.ik_result = tk.StringVar(value="Результат IK...")
        ttk.Label(ik_frame, textvariable=self.ik_result, font=("Consolas", 11)).grid(row=1, column=0, columnspan=7, pady=5)
        ...
        ttk.Button(fk_frame, text="Рассчитать FK", command=self.calculate_fk).grid(row=0, column=6, padx=10)

        self.fk_result = tk.StringVar(value="Результат FK...")
        ttk.Label(fk_frame, textvariable=self.fk_result, font=("Consolas", 11)).grid(row=1, column=0, columnspan=7, pady=5)

        # === Прямое управление сервоприводами (микросекунды) ===
        ...
        ttk.Button(servo_frame, text="Отправить на сервоприводы", 
    ...
    def connect_serial(self):
        try:
            ...
            self.connection_status.config(foreground="green")  # tkinter style
    ...
    def send_command(self, theta):
        ...
        try:
            # Отправляем углы в формате: "A1:123 A2:456 A3:789\n"
            cmd = f"A1:{int(theta[0]):.0f} A2:{int(theta[1]):.0f} A3:{int(theta[2]):.0f}\n"
    ...
    def send_microseconds(self):
        ...
        try:
            us1 = self.servo_vars[0].get()
            us2 = self.servo_vars[1].get()
            us3 = self.servo_vars[2].get()
            
            cmd = f"US1:{us1} US2:{us2} US3:{us3}\n"
            self.ser.write(cmd.encode())
            messagebox.showinfo("Отправлено", f"Микросекунды: {us1}, {us2}, {us3}")
        except Exception as e:
    ...
    def calculate_ik_and_send(self):
        try:
            ...
            result = f"θ1 = {theta[0]:.2f}°  θ2 = {theta[1]:.2f}°  θ3 = {theta[2]:.2f}°"
            ...
            if self.send_command(theta):
                ...
                messagebox.showinfo("Успех", "Команда отправлена на Arduino!")
        except Exception as e:
    ...
    def plot_position(self, pos):
        ...
        self.ax.set_xlabel("X (м)"), self.ax.set_ylabel("Y (м)"), self.ax.set_zlabel("Z (м)")
        self.ax.scatter(pos[0], pos[1], pos[2], color='red', s=120)
        self.ax.text(pos[0], pos[1], pos[2]+0.02, f"({pos[0]:.3f}, {pos[1]:.3f}, {pos[2]:.3f})")
        self.canvas.draw()
