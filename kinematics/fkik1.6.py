...
# Принудительный вывод в stderr (обычно не буферизируется)
        sys.stderr.write(f"DEBUG: angle = {angle_deg:.3f}° → us = {us:.1f} → clamped = {us_clamped}\n")                                                       
        sys.stderr.flush()
        
        return int(round(us_clamped))
