...
class DeltaRobotGUI:
    ...
    def create_widgets(self):
        ...
        ttk.Label(left, text="Delta Robot Control",
        ...
        # IK
        ik_frame = ttk.LabelFrame(left, text="Обратная кинематика (IK)", padding=10)
        ...
        # FK
        fk_frame = ttk.LabelFrame(left, text="Прямая кинематика (FK)", padding=10)
        ...
        traj_frame = ttk.LabelFrame(left,
                     text="Траектория — полином 11-го порядка (4 опорные точки)", padding=10)
        ...
        ttk.Button(traj_frame, text="Построить и показать",
                   command=self.build_trajectory).grid(row=3, column=4, columnspan=2, padx=12, pady=6)
        ...
        stepper_frame = ttk.LabelFrame(left, text="Шаговый двигатель (A4988) — 4 положения", padding=10)
    ...
    def _play_step(self):
        ...
        # двигаем маркер без clear()/полной перерисовки — это убирает рывки и дребезг
